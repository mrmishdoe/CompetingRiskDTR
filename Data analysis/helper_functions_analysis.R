...
eval_DTR = function(data, psi1_hat, psi2_hat, cause_prob, regime){
  ...
  data = data %>% mutate(blip1 = blip_matrix %*% psi1_hat, blip2 = blip_matrix %*% psi2_hat, blip = ifelse(epsilon ==1, blip1, blip2), 
                         opt= as.numeric(blip > 0))
...
AFT.boot = function(n_boot, data, models,save = F, file = "default", regime = "joint", parallel = F, ...){
  ...
  if(regime == "joint"){
    ...
    est = list(psi1_hat = m1$blip, psi2_hat = m2$blip, cause_prob = cause_prob.est)
  }
  ...
  #To make reproducible: get sample of groups before parallelizing
  #Continuous rbind is probably very slow
      
  boot.est = function(i, data, models, groups){
      ...
      res = tryCatch({
         ...
         if(regime == "joint"){
           ...
           psi_censor = aft(boot %>% mutate(delta = ifelse(delta == 1 & epsilon == 1, 1, 0)), 
                            models$treat.mod, models$cens.mod, models$out.mod,1)$blip
           gc()
           
           list(psi_hat = psi_censor)
         }
         else if(regime == "composite"){
           psi_composite = aft(boot %>% mutate(epsilon = 1), models$treat.mod, models$cens.mod,
                               models$out.mod,1)$blip
           gc()
  ...
  if(parallel){
    ...
    boot.est = lapply(1:n_boot, boot.est, data = data, models = models, groups = groups)
  }
...
summaryAFT= function(data, results){
  ...
  if(results$regime == "joint"){
    ...
    for(i in 1:n_boot){
      ...
      eval_w = eval_DTR(data, raw[[i]]$psi1_hat, raw[[i]]$psi2_hat, raw[[i]]$cause_prob, 
                        regime = "weighted")
      eval_g = eval_DTR(data, raw[[i]]$psi1_hat, raw[[i]]$psi2_hat, raw[[i]]$cause_prob, 
                        regime = "greedy")
    ...
    dat.plot = data %>% mutate(oracle = eval_DTR(data, est$psi1_hat, est$psi2_hat, est$cause_prob, 
                                                 regime = "oracle")$blip)
...
plotAFT = function(dat, name){
  ...
  p1 =ggplot(dat1, aes(x = 1:nrow(dat1))) + geom_ribbon(aes(ymin = lower, ymax = upper), fill = "grey70")+ 
    geom_line(aes(y = blip, color = "Chosen Regime"), linewidth= 1.2) + labs(x="Observation number (Ordered)", 
                                                                             y = "Estimated benefit",title ="Cause 1", 
                                                                             color = "Legend",caption = name) + 
    theme(plot.title = element_text(face = "bold"), plot.caption = element_text(hjust = 0.5))+scale_color_manual(values = colors)
  
  p2 =ggplot(dat2, aes(x = 1:nrow(dat2))) + geom_ribbon(aes(ymin = lower, ymax = upper), fill = "grey70")+ 
    geom_line(aes(y = blip,color = "Chosen Regime"), linewidth= 1.2) + labs(x="Observation number (Ordered)", 
                                                                            y = "Estimated benefit",title ="Cause 2",
                                                                            color = "Legend", caption = name) + 
    theme(plot.title = element_text(face = "bold"), plot.caption = element_text(hjust = 0.5))+scale_color_manual(values = colors)
  
  p3 =ggplot(dat, aes(x = 1:nrow(dat))) + geom_ribbon(aes(ymin = lower, ymax = upper), fill = "grey70")+ 
    geom_line(aes(y = blip,color = "Chosen Regime"), linewidth= 1.2) + labs(x="Observation number (Ordered)", 
                                                                            y = "Estimated benefit",title ="Overall",
                                                                            color = "Legend", caption = name) + 
    theme(plot.title = element_text(face = "bold"), plot.caption = element_text(hjust = 0.5))+scale_color_manual(values = colors)
...
ITR_metrics = function(dat, oracle, w){
  dat = dat %>% mutate(opt = as.numeric(blip > 0), oracle_opt = as.numeric(oracle > 0), pot = as.numeric(opt == oracle_opt),
                       val = log(timeto) + (opt - DonorHCV)*oracle)
  
  POT = lm(pot~1,data = dat %>% filter(delta == 1), weights = w)
  val = lm(val~1,data = dat %>% filter(delta == 1), weights = w)
  
  c(POT = coef(POT), Value = coef(val))
}